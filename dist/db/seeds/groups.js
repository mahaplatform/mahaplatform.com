'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _maha = require('maha');

(function () {
  var enterModule = require('react-hot-loader').enterModule;

  enterModule && enterModule(module);
})();

var groupsFixtures = new _maha.Fixtures({
  tableName: 'maha_groups',
  records: [{
    id: 1,
    team_id: 1,
    title: 'Energy',
    created_at: '2017-11-04T21:56:15.893Z',
    updated_at: '2017-11-04T21:56:15.893Z'
  }, {
    id: 2,
    team_id: 1,
    title: 'Environment',
    created_at: '2017-11-04T21:56:15.894Z',
    updated_at: '2017-11-04T21:56:15.894Z'
  }, {
    id: 3,
    team_id: 1,
    title: '4-H Youth Development',
    created_at: '2017-11-04T21:56:15.894Z',
    updated_at: '2017-11-04T21:56:15.894Z'
  }, {
    id: 4,
    team_id: 1,
    title: 'Family and Community Development',
    created_at: '2017-11-04T21:56:15.894Z',
    updated_at: '2017-11-04T21:56:15.894Z'
  }, {
    id: 5,
    team_id: 1,
    title: 'Administration',
    created_at: '2017-11-04T21:56:15.895Z',
    updated_at: '2017-11-04T21:56:15.895Z'
  }, {
    id: 6,
    team_id: 1,
    title: 'Gardening',
    created_at: '2017-11-04T21:56:15.895Z',
    updated_at: '2017-11-04T21:56:15.895Z'
  }, {
    id: 7,
    team_id: 1,
    title: 'Agriculture',
    created_at: '2017-11-04T21:56:15.896Z',
    updated_at: '2017-11-04T21:56:15.896Z'
  }, {
    id: 8,
    team_id: 1,
    title: 'Equity and Inclusion',
    created_at: '2017-11-04T21:56:15.896Z',
    updated_at: '2017-11-04T21:56:15.896Z'
  }, {
    id: 9,
    team_id: 1,
    title: 'Nutrition, Food Access, and Consumer Education',
    created_at: '2017-11-04T21:56:15.898Z',
    updated_at: '2017-11-04T21:56:15.898Z'
  }, {
    id: 10,
    team_id: 1,
    title: 'Human Resources',
    created_at: '2017-11-04T21:56:15.900Z',
    updated_at: '2017-11-04T21:56:15.900Z'
  }, {
    id: 11,
    team_id: 1,
    title: 'Finance',
    created_at: '2017-11-04T21:56:15.902Z',
    updated_at: '2017-11-04T21:56:15.902Z'
  }]
});

var _default = groupsFixtures;
exports.default = _default;
;

(function () {
  var reactHotLoader = require('react-hot-loader').default;

  var leaveModule = require('react-hot-loader').leaveModule;

  if (!reactHotLoader) {
    return;
  }

  reactHotLoader.register(groupsFixtures, 'groupsFixtures', 'src/db/seeds/groups.js');
  reactHotLoader.register(_default, 'default', 'src/db/seeds/groups.js');
  leaveModule(module);
})();

;