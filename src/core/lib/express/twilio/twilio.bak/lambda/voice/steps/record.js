...
const ask = (req, twiml) => {
  ...
  const record = twiml.record({
    action: voiceurl(req, '/voice', { state, action: 'review' }),
    trim: 'trim-silence',
    finishOnKey: '#'
  })
  return {
...
const review = (req, twiml) => {
  ...
  const gather = twiml.gather({
     action: voiceurl(req, '/voice', { state, action: 'confirm', sid }),
     numDigits: 1,
     timeout: duration + 5
   })
   gather.say('You said')
   ...
   twiml.redirect(voiceurl(req, '/voice', { state, action: 'confirm', timeout: true, sid }))
   return {
     verb: 'record',
     action: 'review',
     ask,
...
const processResponse = (req, twiml) => {
  ...
  if(query.timeout) {
    ...
    twiml.redirect(voiceurl(req, '/voice', { state, action: 'ask' }))
    return 'rejected'
  }
}
