const schema = {
  ...
  load: async (knex) => {
    ...
    await knex.schema.createTable('maha_program_members', (table) => {
      table.increments('id').primary()
      table.integer('team_id').unsigned()
      table.integer('program_id').unsigned()
      table.integer('user_id').unsigned()
    ...
    await knex.schema.table('maha_program_members', table => {
      table.foreign('user_id').references('maha_users.id')
      table.foreign('program_id').references('maha_programs.id')
      table.foreign('team_id').references('maha_teams.id')
    })
...
union
    ...
    await knex.raw(`
      ...
      left join crm_consents on (((crm_consents.mailing_address_id = crm_mailing_addresses.id) and (crm_consents.program_id = maha_programs.id) and (crm_consents.type = 'mail'::consent_type))))
