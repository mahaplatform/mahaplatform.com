const schema = {
  ...
  load: async (knex) => {
    ...
    await knex.raw(`
      ...
      from ((drive_access
      join maha_supervisors on (((drive_access."grouping")::text = 'supervisors'::text)))
      join maha_users on ((((drive_access."grouping")::text = 'supervisors'::text) and (maha_users.id = maha_supervisors.user_id))))
      union
      select drive_access.code,
      maha_users.id as user_id,
      drive_access.access_type_id
      from (drive_access
      join maha_users on (((drive_access."grouping")::text = 'everyone'::text)))
      union
      select drive_access.code,
