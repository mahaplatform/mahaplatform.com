...
const shipit = async (args) => {
  ...
  utils.registerTask(shipit, 'deploy:env', async () => {
    await shipit.local(`NODE_ENV=production npx --no-install maha env ${environment}`)
  })

  utils.registerTask(shipit, 'deploy:build', async () => {
    await shipit.local(`NODE_ENV=production npx --no-install maha build ${environment} ${releaseDir}`)
  })
