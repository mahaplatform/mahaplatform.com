...
union
    ...
    await knex.raw(`
      ...
      array_remove(array_agg(distinct crm_responses.form_id), null::integer) as form_ids
      from (crm_contacts crm_contacts_1
      left join crm_responses on ((crm_responses.contact_id = crm_contacts_1.id)))
      group by crm_contacts_1.id
      ), registrations as (
      select crm_contacts_1.id as contact_id,
      array_remove(array_agg(distinct events_registrations.event_id), null::integer) as event_ids
      from (crm_contacts crm_contacts_1
      left join events_registrations on ((events_registrations.contact_id = crm_contacts_1.id)))
      group by crm_contacts_1.id
      ), subscriptions as (
      select crm_contacts_1.id as contact_id,
      array_remove(array_agg(distinct crm_subscriptions.list_id), null::integer) as list_ids
      from (crm_contacts crm_contacts_1
      left join crm_subscriptions on ((crm_subscriptions.contact_id = crm_contacts_1.id)))
      group by crm_contacts_1.id
      ), interests as (
      select crm_contacts_1.id as contact_id,
      array_remove(array_agg(distinct crm_interests.topic_id), null::integer) as topic_ids
      from (crm_contacts crm_contacts_1
      left join crm_interests on ((crm_interests.contact_id = crm_contacts_1.id)))
      group by crm_contacts_1.id
      ), imports as (
      select crm_contacts_1.id as contact_id,
      array_remove(array_agg(distinct maha_imports.id), null::integer) as import_ids
      from ((crm_contacts crm_contacts_1
      left join maha_import_items on ((maha_import_items.object_id = crm_contacts_1.id)))
      left join maha_imports on (((maha_imports.id = maha_import_items.import_id) and ((maha_imports.object_type)::text = 'crm_contacts'::text))))
      group by crm_contacts_1.id
      )
      select crm_contacts.id as contact_id,
      (((((((((((((((jsonb_build_object('full_name', concat(crm_contacts.first_name, ' ', crm_contacts.last_name)) || jsonb_build_object('first_name', crm_contacts.first_name)) || jsonb_build_object('last_name', crm_contacts.last_name)) || jsonb_build_object('email', crm_email_addresses.address)) || jsonb_build_object('phone', crm_phone_numbers.number)) || jsonb_build_object('address', (crm_mailing_addresses.address ->> 'description'::text))) || jsonb_build_object('spouse', crm_contacts.spouse)) || jsonb_build_object('birthday', crm_contacts.birthday)) || jsonb_build_object('organization', crm_contacts.organization)) || jsonb_build_object('position', crm_contacts."position")) || jsonb_build_object('path', concat('/', maha_teams.subdomain, '/crm/contacts/', crm_contacts.id))) || jsonb_build_object('list_ids', subscriptions.list_ids)) || jsonb_build_object('topic_ids', interests.topic_ids)) || jsonb_build_object('form_ids', responses.form_ids)) || jsonb_build_object('event_ids', registrations.event_ids)) || jsonb_build_object('import_ids', imports.import_ids)) as tokens
      from ((((((((((crm_contacts
      join maha_teams on ((maha_teams.id = crm_contacts.team_id)))
...
union
    ...
    await knex.raw(`
      ...
      (jsonb_build_object('path', concat('/', maha_teams.subdomain, '/forms/forms/', crm_responses.form_id, '/responses/', computed.response_id)) || jsonb_object_agg(computed.token, computed.value)) as tokens
      from ((computed
      join crm_responses on ((crm_responses.id = computed.response_id)))
      join maha_teams on ((maha_teams.id = crm_responses.team_id)))
      group by computed.response_id, crm_responses.form_id, maha_teams.subdomain;
    `)
    ...
    await knex.raw(`
      ...
      (jsonb_build_object('path', concat('/', maha_teams.subdomain, '/events/events/', events_registrations.event_id, '/registrations/', computed.registration_id)) || jsonb_object_agg(computed.token, computed.value)) as tokens
      from ((computed
      join events_registrations on ((events_registrations.id = computed.registration_id)))
      join maha_teams on ((maha_teams.id = events_registrations.team_id)))
      group by computed.registration_id, events_registrations.event_id, maha_teams.subdomain;
    `)
...
union
    ...
    await knex.raw(`
      ...
      (jsonb_build_object('path', concat('/', maha_teams.subdomain, '/stores/stores/', stores_orders.store_id, '/orders/', computed.order_id)) || jsonb_object_agg(computed.token, computed.value)) as tokens
      from ((computed
      join stores_orders on ((stores_orders.id = computed.order_id)))
      join maha_teams on ((maha_teams.id = stores_orders.team_id)))
      group by computed.order_id, stores_orders.store_id, maha_teams.subdomain;
    `)
