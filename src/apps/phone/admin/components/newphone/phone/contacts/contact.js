...
class Contact extends React.Component {
  ...
  _handleCall(number, cell) {
    const { contact } = this.props
    const { admin } = this.context
    this.props.onCall({
      through : cell ? admin.user.cell_phone : null,
      number,
      contact
    })
  }
  ...
  _handlePlace(number) {
    this.context.tasks.open({
      items: [
        { label: 'Call with Cell phone', handler: this._handleCall.bind(this, number, true) },
        { label: 'Call with Maha phone', handler: this._handleCall.bind(this, number, false) },
      ]
    })
  }
