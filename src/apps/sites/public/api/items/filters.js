import { BackframeError } from 'maha'
import moment from 'moment'
import _ from 'lodash'

export const applyFilters = (qb, fields, $filters) => {

  const filters = normalizeFilters($filters)

  if(filters.$and) return applyAnd(qb, fields, filters.$and)

  if(filters.$or) return applyOr(qb, fields, filters.$or)
...
const applyAnd = (qb, fields, filters) => {

  filters.map(filter => {
    qb.andWhere(function(builder) {
      applyFilter(builder, Object.keys(filter)[0], fields, Object.values(filter)[0])
    })
  })

}

const applyOr = (qb, fields, filters) => {

  qb.andWhere(function(builder) {
    filters.map(filter => {
      builder.orWhere(function(builder2) {
        applyFilter(builder2, Object.keys(filter)[0], fields, Object.values(filter)[0])
      })
    })
  })
...
const applyFilter = (qb, name, fields, filter) => {

  if(name === '$and') return applyAnd(qb, fields, filter)
