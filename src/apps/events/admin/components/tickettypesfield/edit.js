...
class Edit extends React.PureComponent {
  ...
  _getPriceType() {
    ...
    if(ticket_type.price_type === 'fixed') {
      return [
        { type: 'fields', fields: [
          { label: 'Project', name: 'project_id', type: 'lookup', placeholder: 'Choose a Project', endpoint: '/api/admin/finance/memberships', value: 'id', text: 'display', required: true, format: ProjectToken, defaultValue: ticket_type.project_id },
          { label: 'Revenue Type', name: 'revenue_type_id', type: 'lookup', placeholder: 'Choose a Revenue Type', endpoint: '/api/admin/finance/revenue_types', filter: { id: { $in: [26,49] } }, value: 'id', text: 'display', required: true, format: RevenueTypeToken, defaultValue: ticket_type.revenue_type_id }
        ] },
        ...
        { label: 'Tax Rate', name: 'tax_rate', type: 'ratefield', placeholder: 'Tax Rate', required: true, defaultValue: ticket_type.tax_rate  },
  ...
  _getOverageStrategy() {
    ...
    if(ticket_type.overage_strategy === 'donation') {
      return [
        { label: 'Donation Revenue Type', name: 'donation_revenue_type_id', type: 'lookup', placeholder: 'Choose a Revenue Type', endpoint: '/api/admin/finance/revenue_types', filter: { id: { $in: [30, 37] } }, value: 'id', text: 'display', required: true, format: RevenueTypeToken, defaultValue: ticket_type.donation_revenue_type_id }
      ]
    }
    return []
  }
