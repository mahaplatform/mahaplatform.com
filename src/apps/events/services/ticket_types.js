...
export const updateTicketTypes = async(req, { event, ticket_types }) => {
  ...
  await Promise.mapSeries(add, async (data) => {
    await TicketType.forge({
      ...
      ...whitelist(data, ['name','description','project_id','revenue_type_id','price_type','fixed_price','low_price','high_price','overage_strategy','donation_revenue_type_id','total_tickets','max_per_order','sales_open_at','sales_close_at'])
    }).save(null, {
      transacting: req.trx
    })
  })
