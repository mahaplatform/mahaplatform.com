...
class Performance extends React.Component {
  ...
  _getDevice(quantity, total, is_mobile) {
    ...
    const button = {
      label: percent,
      className: 'link',
      route: `/admin/automation/emails/${email.id}/activities?$filter[$and][0][type][$eq]=open&$filter[$and][1][is_mobile][$eq]=${is_mobile}`
    }
  ...
  _getActivity(value, type) {
    ...
    const button = {
      label: value,
      className: 'link',
      route: `/admin/automation/emails/${email.id}/activities?$filter[$and][0][type][$eq]=${type}`
    }
    return <Button { ...button } />
  }
  ...
  _getButton(value, $filter) {
    ...
    const button = {
      label: value,
      className: 'link',
      route: `/admin/automation/emails/${email.id}/deliveries${query}`
    }
    return <Button { ...button } />
  }
  ...
  _getBounces(value, type) {
    ...
    const button = {
      label: value,
      className: 'link',
      route: `/admin/automation/emails/${email.id}/bounces${query}`
    }
    return <Button { ...button } />
  }
  ...
  _getChart() {
    const { email } = this.props
    return {
      endpoint: `/api/admin/automation/emails/${email.id}/performance`,
