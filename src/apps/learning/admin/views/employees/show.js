...
import { Avatar, List, Page } from '@admin'
...
const Plans = ({ employee, plans }) => {
  ...
  const list = {
    items: plans.map(plan => ({
      content: plan,
      component: PlanToken,
      link: `/admin/learning/plans/${plan.id}`
    })),
    empty: {
      icon: 'list',
      title: 'No plans',
...
const mapResourcesToPage = (props, context) => ({
  employee: `/api/admin/learning/employees/${props.params.id}`,
  plans: `/api/admin/learning/employees/${props.params.id}/plans`
})
