const AddForwardToEmailActivity = {
  ...
  up: async (knex) => {
    ...
    await knex.raw(`
      ...
      select maha_emails.email_campaign_id, count(maha_email_activities.*) as count
      from maha_emails
      left join maha_email_activities on maha_email_activities.email_id=maha_emails.id and maha_email_activities.type='open' and maha_email_activities.is_mobile=true
      where email_campaign_id is not null
      group by email_campaign_id
      ),
      desktop as (
      select maha_emails.email_campaign_id, count(maha_email_activities.*) as count
      from maha_emails
      left join maha_email_activities on maha_email_activities.email_id=maha_emails.id and maha_email_activities.type='open' and maha_email_activities.is_mobile=false
      where email_campaign_id is not null
      group by email_campaign_id
      ),
      clicked as (
