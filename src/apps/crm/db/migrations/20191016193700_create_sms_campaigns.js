const CreateSmsCampaign = {
  ...
  up: async (knex) => {
    await knex.schema.createTable('crm_sms_campaigns', (table) => {
      ...
      table.enum('purpose', ['transactional','marketing'], { useNative: true, enumName: 'crm_sms_campaigns_purpose' })
      table.enum('direction', ['inbound','outbound'], { useNative: true, enumName: 'crm_sms_campaigns_direction' })
      table.enum('status', ['draft','scheduled','sent','active','inactive'], { useNative: true, enumName: 'crm_sms_campaigns_status' })
      table.string('title')
      table.string('code')
      table.string('term')
