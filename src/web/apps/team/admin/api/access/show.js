...
const showRoute = async (req, res) => {
  ...
  const apps = await App.query(qb => {
    qb.select(knex.raw('distinct on (maha_apps.id) maha_apps.*'))
  ...
  const rights = await Right.query(qb => {
    qb.select(knex.raw('distinct on (maha_rights.id) maha_rights.*, maha_users_roles.role_id is not null as assigned'))
