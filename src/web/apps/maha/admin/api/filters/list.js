...
const listRoute = async (req, res) => {
  ...
  const filters = await Filter.query(qb => {
    ...
    qb.whereRaw('maha_filter_accesses.grouping=? or maha_filters.owner_id=? or maha_filter_accesses.user_id=? or maha_users_groups.user_id=?', ['everyone',req.user.get('id'),req.user.get('id'),req.user.get('id')])
