const UpdateAccess = {
  ...
  up: async (knex) => {
    ...
    await knex.raw(`
      ...
      left join "maha_assets" on "maha_assets"."id"="drive_metafiles"."asset_id"
    ) as "items"
    order by "priority"
    `)
    ...
    await knex.raw(`
      ...
      from drive_access
      inner join maha_supervisors on drive_access.grouping = 'supervisors'
      inner join maha_users on drive_access.grouping = 'supervisors' and maha_users.id=maha_supervisors.user_id
      union
      select drive_access.code, maha_users.id as user_id, drive_access.access_type_id
      from drive_access
      inner join maha_users on drive_access.grouping = 'everyone'
      union
