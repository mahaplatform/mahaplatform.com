...
export const createExpense = async (req, params) => {
  ...
  const expense = await Expense.forge({
    ...
    status_id: 1,
    ...whitelist(params, ['user_id','code','date','vendor_id','account_id','project_id','expense_type_id','description','amount'])
  }).save(null, {
    transacting: req.trx
  })
...
export const updateExpense = async (req, expense, params) => {
  ...
  await expense.save({
    ...whitelist(params, ['date','account_id','vendor_id','project_id','expense_type_id','description','amount'])
  }, {
    patch: true,
    transacting: req.trx
  })
  ...
  await completeItem(req, {
    item: expense,
    required: ['date','account_id','vendor_id','receipt_ids','project_id','expense_type_id','description','amount']
  })
