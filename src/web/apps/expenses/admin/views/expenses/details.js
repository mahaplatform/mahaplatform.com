...
const Details = ({ expense, commentUrl }) => {
  ...
  if(expense.line_items.length > 1) {
    ...
    list.items.push({ label: 'Project', content: expense.line_items[0].project.title })
    list.items.push({ label: 'Expense Type', content: expense.line_items[0].expense_type.title })
    list.items.push({ label: 'Description', content: expense.line_items[0].description })
    list.items.push({ label: 'Amount', content: expense.line_items[0].amount })
  }
