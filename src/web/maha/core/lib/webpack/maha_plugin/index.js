...
const collectObjects = (pattern) => [
  ...glob.sync(`src/web/*/${pattern}`),
  ...glob.sync(`src/web/*/${pattern}/index.js`)
]
  ...
  apply(compiler) {
    ...
    const create = (file) => {
      ...
      const reducers = collectObjects('admin/**/reducer.js').map(file => {
        const matches = file.match(/(src\/web\/([^/]*)\/admin\/(.*)\/(.*))\/reducer.js/)
        return {
          ...configs[matches[2]],
          app: matches[2],
      ...
      const variables = {
        ...
        reducers,
        styles: [
          { filepath: path.resolve('src','web','maha','core','packages','reframe','style.less') },
