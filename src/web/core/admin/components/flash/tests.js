...
describe('flash component', () => {
  ...
  describe('component', () => {
    ...
    it('renders with a default state', () => {
      ...
      expect(flash.is('div.maha-flash')).to.be.true
    ...
    it('renders with flash', () => {
      ...
      expect(popup.is('div.maha-flash-popup.success')).to.be.true

      const panel = popup.childAt(0)
      expect(panel.is('div.maha-flash-popup-panel')).to.be.true

      const icon = panel.childAt(0)
      expect(icon.is('div.maha-flash-popup-icon')).to.be.true
      expect(icon.childAt(0).is('i.fa.fa-check-circle')).to.be.true

      const message = panel.childAt(1)
      expect(message.is('div.maha-flash-popup-message')).to.be.true
